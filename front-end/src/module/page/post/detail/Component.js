...
import PostForm from '@/module/form/PostForm/Container'
import I18N from '@/I18N'
import { LG_WIDTH } from '@/config/constant'
import { CVOTE_STATUS, POST_TAG_TYPE } from '@/constant'
...
export default class extends StandardPage {
  ...
  renderAnchors() {
    return (
      <StyledAnchor offsetTop={420}>
        <Anchor.Link
          href="#preamble"
          title={I18N.get('post.fields.preamble')}
        />
        <Anchor.Link
          href="#abstract"
          title={I18N.get('post.fields.abstract')}
        />
        <div style={{ marginTop: 48 }}>
          <Anchor.Link
            href="#goal"
            title={I18N.get('post.fields.goal')}
          />
        </div>
        <Anchor.Link
          href="#motivation"
          title={I18N.get('post.fields.motivation')}
        />
        <Anchor.Link href="#plan" title={I18N.get('post.fields.plan')} />
        <Anchor.Link
          href="#relevance"
          title={I18N.get('post.fields.relevance')}
        />
        <Anchor.Link
          href="#budget"
          title={I18N.get('post.fields.budget')}
        />
      </StyledAnchor>
    )
  }
    ...
    return (
      <div>
        <Meta
          ...
          title={`${detail.title} - Post Detail - Cyber Republic`}
    ...
    return (
      <div>
        ...
        <DescLabel id="preamble">
          {I18N.get('post.fields.preamble')}
        </DescLabel>
        {this.renderPreambleItem(
          I18N.get('post.fields.preambleSub.post'),
          `#${detail.displayId}`
        )}
        {this.renderPreambleItem(
          I18N.get('post.fields.preambleSub.title'),
          detail.title
        )}
        {this.renderPreambleItem(
          I18N.get('post.fields.preambleSub.creator'),
          detail.createdBy.username,
          'username'
        )}
        {this.renderPreambleItem(
          I18N.get('post.fields.preambleSub.status'),
          status
        )}
        {this.renderPreambleItem(
          I18N.get('post.fields.preambleSub.created'),
          moment(detail.createdAt).format('MMM D, YYYY')
        )}
        {sections.map(section => (
          <div key={section}>
            <DescLabel id={section}>
              {I18N.get(`post.fields.${section}`)}
            </DescLabel>
  ...
  renderSocialShareButtonsNode() {
    ...
    return (
      <SocialShareButtons
        shareQuote={`${detail.title} - Post Detail - Cyber Republic`}
      />
    )
  }
  ...
  renderTagsNode() {
    ...
    const res = _.map(tags, tag => {
      ...
      if (type === POST_TAG_TYPE.INFO_NEEDED) {
        return (
          <div key={_id} style={{ display: 'inline' }}>
            ...
            >
              {I18N.get(`post.tag.type.${type}`)}
              {'  '}
            </LabelPointer>
            <Modal
              title={I18N.get(`post.tag.type.${type}`)}
            ...
            >
              <div style={{ fontWeight: 200, paddingBottom: '18px' }}>
                {I18N.get('post.modal.pleaseUpdate')}
              </div>
              {I18N.get('post.modal.commentsFromCouncil')}
              <br />
      ...
      if (type === POST_TAG_TYPE.UNDER_CONSIDERATION) {
        return (
  ...
  renderCouncilActionsNode() {
    ...
    const createFormBtn = isCouncil && (
      <Col xs={24} sm={8}>
        ...
        >
          {I18N.get('post.btn.makeIntoProposal')}
        </StyledButton>
      </Col>
    )
    const needDueDiligenceBtn = isCouncil && (
      <Col xs={24} sm={8}>
        ...
        >
          {I18N.get('post.btn.needDueDiligence')}
        </StyledButton>
      </Col>
    )
    const needAdvisoryBtn = isCouncil && (
      <Col xs={24} sm={8}>
        ...
        >
          {I18N.get('post.btn.needAdvisory')}
        </StyledButton>
      </Col>
    )
  ...
  consider = async () => {
    ...
    try {
      await this.props.addTag({
        id: _id,
        type: POST_TAG_TYPE.UNDER_CONSIDERATION
      })
      this.refetch()
      message.success(I18N.get('post.msg.consideration'))
    } catch (error) {
      logger.error(error)
    }
  }
  ...
  showAddTagModal = () => {
    Modal.confirm({
      title: I18N.get('post.modal.addTagComment'),
      content: <TextArea onChange={this.onCommentChanged} />,
      okText: I18N.get('post.modal.confirm'),
      cancelText: I18N.get('post.modal.cancel'),
  ...
  renderEditForm = () => {
    ...
    const props = {
      ...
      header: I18N.get('post.header.edit'),
      data: detail
    }
    ...
    return (
      <Modal
        ...
        width="70%"
      >
        {this.state.showForm && <PostForm {...props} />}
      </Modal>
    )
  }
  ...
  makeIntoPropose = async () => {
    ...
    try {
      const res = await proposePost(param)
  ...
  needDueDiligence = async () => {
    ...
    message.success(I18N.get('post.msg.notify'))
  }
